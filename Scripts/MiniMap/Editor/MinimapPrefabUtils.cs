...
public static class MapPrefabUtils
{
    ...
    private const string _spriteFolderName = "MapSprites";
    private const string _spriteNameMask = "map_sprite*%.png";
    private const string _prefabNameask = "map_prefab.prefab";
    ...
    static MapPrefabUtils()
    {
        ...
        _spriteFolder = Path.Combine(_prefabFolder, _spriteFolderName);
    ...
    )
    {
        ...
        CreateDirectoryIfNoteExists(_prefabFolder);
        ...
        CreateDirectoryIfNoteExists(_prefabRelativeFolder);
        ...
        PrefabUtility.SaveAsPrefabAssetAndConnect(rootRectTransform.gameObject, Path.Combine(_prefabRelativeFolder, _prefabNameask),
        ...
        Texture2D[] screens, float screenScale)
    {
        ...
        string spriteName = _spriteNameMask
            .Replace("*", x.ToString())
            .Replace("%", z.ToString());
        ...
        if (!partTexture)
        {
            string missingTextureName = _screenshotsFileName.Replace("*", textureIndex.ToString());
    ...
    {
        CreateDirectoryIfNoteExists(_spriteFolder);

        string relativeSpritePath = Path.Combine(_prefabRelativeFolder, _spriteFolderName, spriteName);
